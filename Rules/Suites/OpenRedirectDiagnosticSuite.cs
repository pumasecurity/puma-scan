...
namespace Puma.Security.Rules.Suites
{
    ...
    public class OpenRedirectDiagnosticSuite : BaseSyntaxDiagnosticSuite
    {
        public OpenRedirectDiagnosticSuite()
        {
            Analyzers = new IAnalyzer[]
            {
                Container.Resolve<ResponseRedirectAnalyzer>(),
                Container.Resolve<MvcRedirectAnalyzer>()
            }.ToImmutableArray();
        }
    }
}