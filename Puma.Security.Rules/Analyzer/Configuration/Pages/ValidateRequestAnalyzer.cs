...
{
    [SupportedDiagnostic(DiagnosticId.SEC0012)]
    internal class ValidateRequestAnalyzer : BaseConfigurationFileAnalyzer, IConfigurationFileAnalyzer
    {
        ...
        public void OnCompilationEnd(PumaCompilationAnalysisContext pumaContext)
        {
            foreach (var config in ConfigurationFiles)
            {
                ...
                var attribute = element?.Attribute("validateRequest");
