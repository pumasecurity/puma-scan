...
{
    [SupportedDiagnostic(DiagnosticId.SEC0015)]
    internal class HttpOnlyAnalyzer : BaseConfigurationFileAnalyzer, IConfigurationFileAnalyzer
    {
        ...
        public void OnCompilationEnd(PumaCompilationAnalysisContext context)
        {
            foreach (var config in ConfigurationFiles)
            {
                ...
                //Get the httpOnlyCookies attribute
                var attribute = element.Attribute("httpOnlyCookies");
